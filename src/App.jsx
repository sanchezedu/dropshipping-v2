...
import { X, Mail, Gift, ChevronRight, GitCompare, Info } from 'lucide-react';
...
function App() {
  ...
  const handleNavigate = (page, product = null) => {
    ...
    if (product) {
      ...
      localStorage.setItem('dropshop-recent', JSON.stringify([product, ...filtered].slice(0, 10)));
    }
    window.scrollTo(0, 0);
  };
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={i} className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl">
                <div className="w-12 h-12 bg-indigo-100 rounded-xl flex items-center justify-center flex-shrink-0"><svg className="w-6 h-6 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={item.icon} /></svg></div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { name: 'Andrea Paz', location: 'Quito', text: 'Me encanto la rapidez del envio. Llego en 3 dias. El producto es exactamente como en las fotos.', rating: 5, avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100' },
              { name: 'Marco Delgado', location: 'Guayaquil', text: 'Segunda vez que compro y siempre todo excelente. El servicio es muy atento.', rating: 5, avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100' },
              { name: 'Sofia Mendoza', location: 'Cuenca', text: 'Me llego el pedido muy bien empacado. Precios muy competitivos.', rating: 4, avatar: 'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=100' }
            ].map((t, i) => (
              <div key={i} className="bg-gray-50 p-6 rounded-2xl">
                <div className="flex items-center gap-3 mb-4"><img src={t.avatar} alt={t.name} className="w-12 h-12 rounded-full object-cover" /><div><p className="font-bold">{t.name}</p><p className="text-xs text-gray-500">{t.location}</p></div></div>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid md:grid-cols-4 gap-8">
            <div>
              ...
              <p className="text-gray-400 text-sm">Tu tienda de confianza para compras en linea.</p>
              <div className="flex gap-3 mt-4">
                <a href="https://facebook.com" target="_blank" rel="noreferrer" className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center hover:bg-blue-600 transition-colors">f</a>
                <a href="https://instagram.com" target="_blank" rel="noreferrer" className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center hover:bg-pink-600 transition-colors">i</a>
                <a href="https://wa.me/593991234567" target="_blank" rel="noreferrer" className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center hover:bg-green-600 transition-colors">w</a>
              </div>
            </div>
            <div>
          ...
          <div className="border-t border-gray-800 mt-8 pt-8 text-center text-sm text-gray-500">© 2026 DropShop Ecuador. Todos los derechos reservados.</div>
        </div>
      </footer>

      {/* WhatsApp */}
      <a href="https://wa.me/593991234567" target="_blank" rel="noreferrer" className="fixed bottom-6 right-6 z-50 bg-green-500 text-white w-14 h-14 rounded-full flex items-center justify-center shadow-lg hover:bg-green-600 transition-all">
