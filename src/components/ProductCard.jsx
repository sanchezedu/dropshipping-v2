  ...
  return (
    <div className="product-card group relative bg-white rounded-2xl shadow-md hover:shadow-2xl transition-all duration-300 overflow-hidden">
      ...
      <div className="absolute top-0 left-0 right-0 z-10 flex justify-between items-start p-3">
        ...
        {tags.includes('bestseller') && <span className="bg-gradient-to-r from-yellow-500 to-orange-500 text-white px-3 py-1.5 rounded-lg text-sm font-bold">Top</span>}
        {tags.includes('nuevo') && <span className="bg-gradient-to-r from-green-500 to-emerald-600 text-white px-3 py-1.5 rounded-lg text-sm font-bold">Nuevo</span>}
      </div>
      ...
      <div onClick={() => onNavigate('product', product)} className="p-4 cursor-pointer">
        <p className="text-sm text-indigo-600 font-medium mb-1 capitalize">{category}</p>
        <h3 className="font-semibold text-gray-800 mb-2 line-clamp-2">{name}</h3>
        
        <div className="flex items-center justify-between mb-2">
          <div className="flex">{[...Array(5)].map((_, i) => <Star key={i} className={`w-4 h-4 ${i < Math.floor(rating) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`} />)}</div>
          <span className="text-xs text-green-600">{reviews.toLocaleString()} ventas</span>
        </div>

        <div className="flex items-center gap-2">
          <span className="text-xl font-bold text-indigo-600">${price.toFixed(2)}</span>
          {oldPrice > price && <span className="text-sm text-gray-400 line-through">${oldPrice.toFixed(2)}</span>}
        </div>
