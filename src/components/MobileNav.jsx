...
export default function MobileNav({ currentPage, onNavigate, cartCount, wishlistCount }) {
  const navItems = [
    ...
    { id: 'cart', icon: ShoppingCart, label: 'Carrito', count: cartCount },
    { id: 'wishlist', icon: Heart, label: 'Favoritos', count: wishlistCount },
    { id: 'user', icon: User, label: 'Cuenta' },
  ];

  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-800 border-t dark:border-slate-700 shadow-[0_-4px_20px_rgba(0,0,0,0.1)] md:hidden z-50">
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center flex-1 h-full relative transition-colors ${
                isActive ? 'text-indigo-600' : 'text-gray-400 dark:text-slate-500'
              }`}
            >
